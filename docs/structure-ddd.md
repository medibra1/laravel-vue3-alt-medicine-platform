...
│       ├── Billing/...
│       └── Reporting/...
└── e2e/                               # Playwright — golden-path navigateur, pas Pest/Vitest (npm run test:e2e)
    ├── *.spec.ts
    ├── global-setup.ts / global-teardown.ts  # shell out vers tinker pour préparer/nettoyer les données
    └── run-tinker.ts
...
├── Components/
│   ├── App/                           # wrappers Vuetify génériques (AppButton, AppTabs, AppCard...) — seul point d'import direct de vuetify
│   └── {Domain}/                      # composants métier composés, propres à un domaine (ex. Patients/TreatmentCard.vue) — pas de wrapper générique
└── Pages/Admin/{Domain}/
